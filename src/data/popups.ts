 ...
 * ## 정보를 어떻게 채우나
 *
 * **링크만 걸어두지 않고 가져와서 정리해요.** 네이버 링크만 주면 사용자가 검색
 * 결과를 다시 헤매야 하니까, 기간·장소·시간·입장료·프로그램을 우리 화면에 적어둬요.
 *
 * 받아오는 방법은 검색 그라운딩(`src/lib/ai/web-research.ts`)이에요. 캐치테이블·
 * 네이버 HTML 은 긁지 않아요 — 약관 문제도 있고, 두 곳 다 로그인·봇 차단이 걸린
 * JS 페이지라 서버에서 긁으면 금방 막혀요.
 *
 * 틀리면 헛걸음이 되는 정보라서 정리해둔 값에는 항상 두 가지가 붙어요.
 *  - **출처** — 원문에서 직접 확인할 수 있게
 *  - **확인 날짜** — 언제 기준 정보인지 알 수 있게 (`updatedAt`)
 *
 * 링크는 이제 정보 통로가 아니라 **예약**과 **원문 확인**용이에요.
 */
