...
> whisky + wikipedia. 위스키 사전 312병, 용어 사전, 증류소 지도, 제보 시세까지 모여 있어서
...
취향 진단 질문은 12개예요. 축 점수는 질문 수에 맞춰 자동으로 눈금이 조정돼요
...
`src/data/whiskies.ts` 에 500병이 있어요. 국내에서 구할 수 있는 병 위주고, 나라별로는
스코틀랜드 292 · 미국 66 · 일본 46 · 아일랜드 39 · **프랑스 22** · 대만 7 · 인도 7 · 한국 5 순이에요.
...
### AI 로 팝업 찾기 (지금은 꺼져 있어요)

> `src/data/features.ts` 의 `FEATURES.popup` 이 `false` 예요. 정보를 어디서 어떻게 받아올지 더
> 정하기로 해서 잠시 껐어요. 코드는 그대로 있으니 그 한 줄만 `true` 로 바꾸면 메뉴·화면이 돌아와요.
